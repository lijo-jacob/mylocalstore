...
public class AdvancedQueryParser extends QueryParserBase<SolrDataQuery> {
	...
	public final SolrQuery doConstructSolrQuery(SolrDataQuery query) {
		...
		if(query instanceof AdvancedFacetQuery) {
			processAdvancedFacetOptions(solrQuery, (AdvancedFacetQuery) query); 
		}
	...
	private void processAdvancedFacetOptions(SolrQuery solrQuery, AdvancedFacetQuery query) {
		if (enableFaceting(solrQuery, query)) {
			appendAdvancedFacetingOnFields(solrQuery, (AdvancedFacetQuery) query);
		}
	}
	...
	private void appendAdvancedFacetingOnFields(SolrQuery solrQuery, AdvancedFacetQuery query) {
		List<RangeFacet> rangeFacets = query.getRangeFacets();
		if(null == rangeFacets) {
			return;
		}
		List<String> rangeFields = new ArrayList<String>();
		for(RangeFacet rangeFacet : rangeFacets) {
			rangeFields.add(rangeFacet.getField());
			String rangeStartParamName = "f." + rangeFacet.getField() + "." + FacetParams.FACET_RANGE_START;
			solrQuery.setParam(rangeStartParamName, rangeFacet.getStart());
			String rangeEndParamName = "f." + rangeFacet.getField() + "." + FacetParams.FACET_RANGE_END;
			solrQuery.setParam(rangeEndParamName, rangeFacet.getEnd());
			String rangeGapParamName = "f." + rangeFacet.getField() + "." + FacetParams.FACET_RANGE_GAP;
			solrQuery.setParam(rangeGapParamName, rangeFacet.getGap());
			String rangeOtherParamName = "f." + rangeFacet.getField() + "." + FacetParams.FACET_RANGE_OTHER;
			List<FacetRangeOther> rangeOthers = rangeFacet.getOther();
			if(null != rangeOthers) {
				List<String> rangeOtherList = new ArrayList<String>();
				for(FacetRangeOther rangeOther : rangeOthers) {
					rangeOtherList.add(rangeOther.toString());
				}
				solrQuery.setParam(rangeOtherParamName, rangeOtherList.toArray(new String[rangeOtherList.size()]));
			}
		}
		solrQuery.setParam(FacetParams.FACET_RANGE, rangeFields.toArray(new String[rangeFields.size()]));
	}
